// The body of every app's `instrumentation.ts` preload. Deliberately NOT re-exported from the
// tracing barrel: it pulls the whole OTel SDK and Sentry, and the barrel is imported by ordinary
// request-path code.
...
import { scrubPii } from '../error-tracking/scrub-pii';
...
/**
 * Runs before Nest's ConfigModule, hence the raw `process.env`. `defaultServiceName` is the app's
 * own name, used when OTEL_SERVICE_NAME is unset — the one thing that differs between deployables.
 */
export function startTelemetry(defaultServiceName: string): void {
