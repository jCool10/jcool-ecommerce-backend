...
import type { DomainEventDispatcherPort, EventLabelRegistry } from './domain-event-dispatcher.port';
...
function build({
  ...
  const processor = new DomainEventProcessor(
    ...
    { claim },
    // `satisfies`, not a cast: both ports are one method wide, so a spy object conforms structurally
    // and the compiler checks that it does — a cast would let a signature change land here silently.
    { dispatch } satisfies DomainEventDispatcherPort,
    { label } satisfies EventLabelRegistry,
    logger,
  );
