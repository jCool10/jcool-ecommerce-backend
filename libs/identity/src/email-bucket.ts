...
/** Length is not entropy, but a short key is brute-forceable from a handful of self-registered
 * (email, bucket) pairs, which reopens the oracle the HMAC closes. */
export const MIN_BUCKET_KEY_LENGTH = 32;

/**
 * Keyed HMAC rather than a plain hash because `users.id` is returned by the public
 * `POST /auth/register` and `/auth/me`: an unkeyed digest would publish 12 bits of `H(email)` per
 * id, an offline email-confirmation oracle. The key is a parameter, not config, so this stays pure.
 */
...
/** Row-independent fingerprint of the bucket key: it holds on a 0-row database, which the row canary
 * cannot cover, and catches a key that was wrong from the very first row. */
