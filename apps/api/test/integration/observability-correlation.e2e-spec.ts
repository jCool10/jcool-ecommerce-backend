...
describe('Correlation id (integration)', () => {
  ...
  it('stamps the correlation id on a 4xx error envelope + header, keeping the legacy fields', async () => {
    // An out-of-range page fails the ValidationPipe (400) on a public route before any handler/DB —
    // exercises the exception-filter enrichment path.
  ...
  it('serves the public /debug/boom route as a masked 5xx envelope (error pipeline)', async () => {
    // The debug endpoint throws on purpose; it is @Public() and gated to non-prod. This drives the
    // same filter path that reports to Sentry (a no-op without a DSN), proving the 5xx envelope
