...
export default defineConfig({
  test: {
    ...
    // Workspace packages from source, then the tsconfig path aliases. A key also matches every path
    // under it, so a subpath goes before its package and the `@` catch-all goes last.
    alias: {
