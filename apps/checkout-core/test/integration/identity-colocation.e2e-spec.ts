...
import { BUCKET_COUNT, IdentityService, bucketOf } from '@jcool/identity';
...
// Resolved, not `node_modules/.bin/tsx` under the package: workspaces hoist the bin to the repo root.
const tsxCli = createRequire(join(packageRoot, 'package.json')).resolve('tsx/cli');
  ...
  async function runOperatorScan(): Promise<string> {
    try {
      const { stdout } = await execFileAsync(
        process.execPath,
        [tsxCli, '--conditions=@jcool/source', 'scripts/verify-identity-buckets.ts'],
        {
          cwd: packageRoot,
