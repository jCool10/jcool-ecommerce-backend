...
// The consumer of the handle published here — TelemetryFlushService — is a library and is tested as
// one; these cases cover the half only this preload owns: the flush budget, and the two sinks racing
// rather than chaining.
describe('instrumentation preload', () => {
  ...
  async function withInstrumentation(
    ...
    try {
      ...
      if (!flush) throw new Error('instrumentation published no flush handle');
