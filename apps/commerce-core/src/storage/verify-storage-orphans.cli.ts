 ...
 * when anything is found, so a scheduled run fails loudly. Lives under the app rather than `scripts/`
 * because `scripts/` is excluded from the build and `tsx` is a devDependency, so a `scripts/`
 * entrypoint cannot run in the image — and under the app rather than `libs/storage` because it reads
 * two of this app's tables.
 */
...
import { mediaAssets, productImages } from '../database/schema';
