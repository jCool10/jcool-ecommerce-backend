...
describe('CsrfTokenService', () => {
  ...
  // Cookies the api issued are still out there at cutover; the derivation is the contract.
  it("accepts a token signed under the api's key derivation", () => {
    const key = createHmac('sha256', SECRET).update('csrf-double-submit-v1').digest();
    const token = `issued-by-api.${createHmac('sha256', key).update('issued-by-api').digest('base64url')}`;
