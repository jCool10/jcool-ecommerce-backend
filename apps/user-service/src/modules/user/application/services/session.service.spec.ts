...
describe('SessionService', () => {
  ...
  describe('revokeAllThen', () => {
    it('writes only after the refresh tokens are revoked and the epoch is bumped', async () => {
      const seen: string[][] = [];

      await service.revokeAllThen('u1', () => {
        seen.push([...repo.revokedAllFor, ...epoch.bumpedFor]);
        return Promise.resolve();
      });
    ...
    it('still writes when only the publish failed, then surfaces that failure', async () => {
      ...
      epoch.bump = () => Promise.reject(unpublished);
      let wrote = false;

      await expect(
        service.revokeAllThen('u1', () => {
          wrote = true;
          return Promise.resolve();
        }),
      ).rejects.toBe(unpublished);
      expect(wrote).toBe(true);
    });

    it('does not write when the bump itself failed', async () => {
      epoch.bump = () => Promise.reject(new Error('db down'));
      let wrote = false;
