...
const config = fakeConfigService({ 'auth.refreshTokenTtl': '7d' });

// The token is the claims it was asked to sign; signing itself is the signer's spec.
const signer: AccessTokenSignerPort = {
  expiresIn: 900,
  sign: (claims) => Promise.resolve(JSON.stringify(claims)),
};
const jwt = { verify: <T>(token: string): T => JSON.parse(token) as T };
...
describe('AuthTokensService', () => {
  ...
  beforeEach(() => {
    ...
    service = new AuthTokensService(signer, config, repo);
  });
  ...
  it('signs an access JWT carrying sub + role + a unique jti + the session epoch', async () => {
    ...
    const payload = jwt.verify<AccessTokenClaims>(accessToken);
  ...
  it("returns the signer's lifetime as expiresIn", async () => {
