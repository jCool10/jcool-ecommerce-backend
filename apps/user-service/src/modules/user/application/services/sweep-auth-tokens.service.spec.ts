...
function build() {
  const config = fakeConfigService({ 'retention.authTokenGraceDays': 7, 'retention.refreshTokenGraceDays': 30 });
  ...
  return { make, emailVerification, passwordReset, refresh };
}
...
describe('SweepAuthTokensService', () => {
  ...
  it('recomputes the cutoff on every sweep', async () => {
