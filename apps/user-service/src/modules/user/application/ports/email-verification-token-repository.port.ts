...
export interface EmailVerificationTokenRepositoryPort {
  ...
  /** Called once a fresh token is stored, so a failed resend leaves the previous one live. */
  invalidateOthersForUser(userId: string, keptTokenHash: string): Promise<void>;
