...
| [`RUNBOOK.md`](../RUNBOOK.md) | Manual operator procedures with consequences — key handling, backup and restore, deploying Elasticsearch, reindex, DLQ replay, bucket reconciliation, retention, owed refunds, out-of-band migrations | To perform an operation against a live system |
...
| [`adr-search-index-consistency.md`](./adr-search-index-consistency.md) | Why the search index follows the catalog through the transactional outbox with engine-side versioning, the alternatives rejected, the invariants it holds and when to revisit it | To change how catalog writes reach search, or to judge whether the design still fits |
