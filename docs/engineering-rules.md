...
What a change has to satisfy to be accepted here. The reasoning behind each individual gate lives next to the gate itself — `.github/workflows/ci.yml`, and `.dependency-cruiser.cjs`, `eslint.config.mjs` and `vitest.config.mts` in `apps/api/`, are all commented at the point of enforcement. This file records the judgment calls those files cannot state about themselves.
...
```bash
pnpm lint:check && pnpm typecheck && pnpm arch:check && pnpm build
pnpm test:cov    # unit tier, with the coverage floor
pnpm test:e2e    # integration tier, needs Docker
```
...
Schema changes are generated, never typed into an existing file: edit the Drizzle schema, run `pnpm db:generate`, and commit the emitted `.sql` together with its `meta/_journal.json` entry.
