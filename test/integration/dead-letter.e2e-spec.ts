...
describe('Retry, backoff and dead-letter queue (integration, real Postgres + Redis)', () => {
  ...
  it('applies the message on replay once the handler is fixed, and clears the dead letter', async () => {
    ...
    effect.mockRestore();
    const summary = await replayDeadLetters(queue, dlq, { dryRun: false });
  ...
  it('replays a message that turned out to be applied without applying it twice', async () => {
    ...
    await replayDeadLetters(queue, dlq, { dryRun: false });

    // Waiting on `completed` rather than on the absence of an effect: the replayed delivery has to
    // have actually been consumed before "no second effect" means anything.
    await vi.waitFor(async () => expect(await queue.getJobCounts('completed')).toMatchObject({ completed: 1 }), {
      timeout: 15_000,
      interval: 50,
    });
    // The inbox is what makes replay safe: same outbox id, so the second application is collapsed.
  ...
  it('changes nothing on a dry run, so an operator can look before replaying', async () => {
    ...
    const summary = await replayDeadLetters(queue, dlq);
