...
describe('Inventory pessimistic reserve (integration, real Postgres)', () => {
  ...
  // Reserve inside a transaction, mirroring how place-order will call the port. A
  // thrown InsufficientStockError rolls the whole unit of work back.
  async function reserve(orderId: string, lines: ReserveLine[]): Promise<void> {
    await db.transaction(async (tx) => {
      await repo.reservePessimistic(tx, orderId, lines);
    });
  }
  ...
  it('holds stock when available: reserved += qty, version bumped, one HELD row', async () => {
  ...
  it('throws InsufficientStockError and rolls back when short (DB unchanged)', async () => {
  ...
  it('holds multiple lines of one order regardless of input order', async () => {
    ...
    // Deliberately out of variantId order — the repo sorts before locking.
