...
describe('Payment webhook → order finalization (integration, real Postgres)', () => {
  ...
  async function openPayment(): Promise<{ token: string; orderId: string; sessionId: string; variantId: string }> {
    ...
    return { token, orderId, sessionId: pay.body.providerSessionId as string, variantId };
  }
  ...
  const paid = (sessionId: string, eventId: string, paymentIntent = 'pi_e2e') =>
    signWebhook({ secret: WEBHOOK_SECRET, event: checkoutSessionCompleted(sessionId, { eventId, paymentIntent }) });
  ...
  it('paid webhook finalizes the order PAID and commits the hold in one move', async () => {
    ...
    const res = await postWebhook(paid(sessionId, 'evt_paid_1'));
  ...
  it('duplicate delivery (same event id) never finalizes twice — stock committed once', async () => {
    const { orderId, sessionId, variantId } = await openPayment();
    const signed = paid(sessionId, 'evt_dup_1');
  ...
  it('failed AFTER paid does not regress: order stays PAID, stock unchanged', async () => {
    const { orderId, sessionId, variantId } = await openPayment();
    await postWebhook(paid(sessionId, 'evt_win')).then((r) => expect(r.status).toBe(200));
  ...
  it('paid AFTER failed does not regress: order stays FAILED, stock stays released', async () => {
    ...
    const late = await postWebhook(paid(sessionId, 'evt_paid_late'));
  ...
  it('webhook for an unknown session (arrived before the order) acks 2xx and finalizes nothing', async () => {
    ...
    const res = await postWebhook(paid('cs_orphan_session', 'evt_orphan'));
  ...
  it('is exactly-once under CONCURRENT paid deliveries: one finalize, stock committed once', async () => {
    const { orderId, sessionId, variantId } = await openPayment();
    const signed = paid(sessionId, 'evt_race_1');
