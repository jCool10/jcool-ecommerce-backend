...
describe('Payment webhook → order finalization (integration, real Postgres)', () => {
  ...
  it('paid webhook finalizes the order PAID and commits the hold in one move', async () => {
    ...
    const res = await postWebhook(app, paid(sessionId, charge, 'evt_paid_1'));
    ...
    expect((await readOrder(app, orderId)).paymentRef).toBe('pi_e2e');
  });
  ...
  it('a completed session whose payment has not cleared hands over nothing — no PAID order, no stock', async () => {
    ...
    const res = await postWebhook(app, paid(sessionId, { ...charge, paymentStatus: 'unpaid' }, 'evt_unpaid'));
  ...
  it('a success reporting an amount we never charged is refused — order and stock untouched', async () => {
    ...
    const res = await postWebhook(app, paid(sessionId, { ...charge, amountMinor: 1 }, 'evt_wrong_amount'));
  ...
  it('settles once the async payment clears — the earlier unpaid notice did not burn the event stream', async () => {
    const { orderId, sessionId, variantId, charge } = await openPayment();
    await postWebhook(app, paid(sessionId, { ...charge, paymentStatus: 'unpaid' }, 'evt_async_pending')).expect(200);

    const res = await postWebhook(app, paid(sessionId, charge, 'evt_async_cleared'));
  ...
  it('duplicate delivery (same event id) never finalizes twice — stock committed once', async () => {
    ...
    expect((await readOrder(app, orderId)).status).toBe('PAID');
    expect((await readStock(app, variantId)).quantityOnHand).toBe(STOCK - 1); // not STOCK - 2
  });

  it('failed AFTER paid does not regress: order stays PAID, stock unchanged', async () => {
    const { orderId, sessionId, variantId, charge } = await openPayment();
    await postWebhook(app, paid(sessionId, charge, 'evt_win')).then((r) => expect(r.status).toBe(200));

    const late = await postWebhook(app, expired(sessionId, 'evt_late_fail'));
  ...
  it('paid AFTER failed does not regress: order stays FAILED, stock stays released', async () => {
    const { orderId, sessionId, variantId, charge } = await openPayment();
    await postWebhook(app, expired(sessionId, 'evt_fail_first')).then((r) => expect(r.status).toBe(200));

    const late = await postWebhook(app, paid(sessionId, charge, 'evt_paid_late'));
  ...
  it('failed webhook finalizes the order FAILED and releases the hold', async () => {
    ...
    const res = await postWebhook(app, expired(sessionId, 'evt_fail_1'));
  ...
  it('webhook for an unknown session (arrived before the order) acks 2xx and finalizes nothing', async () => {
    ...
    expect((await readOrder(app, orderId)).status).toBe('PENDING'); // the real order is untouched
  });
  ...
  it('is exactly-once under CONCURRENT paid deliveries: one finalize, stock committed once', async () => {
    ...
    expect((await readOrder(app, orderId)).status).toBe('PAID');
    expect((await readStock(app, variantId)).quantityOnHand).toBe(STOCK - 1); // committed once, not twice
  });
});
