...
| Problem | Approach | Where |
| --- | --- | --- |
| **Sharding-ready user ids** | Every user-context id is a signed 63-bit integer in a Postgres `bigint`, laid out `41 ts_ms │ 12 bucket │ 5 node │ 5 seq` over an epoch of 2026-01-01. The 12-bit routing bucket is `HMAC(IDENTITY_BUCKET_KEY, normalized_email) mod 4096` — derived from the same normalized email the `UNIQUE(email)` index sees, so a future shard split routes from the id alone, with no lookup table, and email uniqueness survives it | `packages/id-codec/src/snowflake.codec.ts` |
| **An integer id that travels as a string** | 63 bits outruns the 53 a JSON number carries exactly, about 25 days past the epoch, so an id crosses the wire as a decimal string and nothing applies `Number()` to one. Drizzle's two `bigint` modes are both wrong here — `number` truncates silently, `bigint` makes `JSON.stringify` throw — so the column is a `customType` whose TypeScript type is `string`. node-postgres already hands back `int8` as a string, so both directions are identity and every `userId: string` signature in the repo stayed as it was | `packages/platform/src/database/snowflake-id.column.ts` |
| **Why HMAC, not a hash** | `users.id` is public. An unkeyed digest would turn every published id into an offline oracle for "does this address have an account here". The key is **permanent**: the user-service's database pins its fingerprint, and the id layout it was built under, on first boot, and refuses a later boot under either a different key or a different layout | `apps/user-service/src/modules/user/infrastructure/identity-bucket-key.verifier.ts` |
...
- **No `CHECK` on the id layout.** The column is a plain `bigint`; the codec rejects anything outside the encodable range on decode, but a raw-SQL writer is not blocked at the database. The honest reason is that nothing writes those tables but this process.
- **32 ids per node-millisecond, and 30 leasable nodes, are real ceilings.** They are what 63 bits leave after 41 for the timestamp and 12 for the bucket. A single replica saturates at 32,000 ids/s and a request may ask for at most 32 (one node-millisecond — a larger batch would busy-wait into the next millisecond and block the event loop); across nodes, the fleet tops out near a million a second. The node pool is the tighter one: a rolling deploy holds two nodes per replica plus quarantine, so more than ~12 id-service replicas needs bits taken from `seq` or `ts` first, and that is a layout change the database pin will refuse until the layout version is bumped.
