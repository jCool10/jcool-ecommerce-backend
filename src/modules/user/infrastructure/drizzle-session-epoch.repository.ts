...
/**
 * Drizzle adapter for the session epoch on `users.token_epoch`. `bump` is a single
 * atomic `token_epoch + 1`, so concurrent logout-all calls can't lose an increment.
 */
@Injectable()
