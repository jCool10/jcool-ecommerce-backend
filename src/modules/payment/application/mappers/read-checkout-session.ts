/**
 * Pull the fields we act on out of a verified Stripe-style Checkout Session event body. The body is
 * authenticated but its inner shape is the sender's, so every field is optional and a missing or
 * wrongly-typed one yields undefined rather than throwing — each caller decides what absence means.
 */
export interface CheckoutSessionFacts {
  /** Session handle, matched against `Payment.providerSessionId`. */
  sessionId?: string;
  /** PaymentIntent handle, recorded on settle to link session → intent. */
  intentId?: string;
