...
// Media schema (uploadable objects and their lifecycle) — infrastructure, never imported by domain.
// No FK to products: the reference points the other way, and cross-context FKs are not used here.
...
export const mediaAssets = pgTable(
  'media_assets',
  {
    ...
    // the value comes from the bucket rather than from us. Read as a number via the mode below.
  ...
  (t) => [
    // Covers the sweep's claim exactly: the same status set, the same column it orders by. Partial,
    // so it stays proportional to what is reclaimable rather than to every asset ever uploaded.
    ...
    // between deleting the object and deleting the row leaves one behind to be found again.
