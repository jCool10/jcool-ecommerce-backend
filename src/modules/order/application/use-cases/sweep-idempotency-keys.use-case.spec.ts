...
/**
 * That a `COMPLETED` row inside its TTL survives is proved against real SQL in
 * `retention-sweep.e2e-spec.ts` — the predicate is the store's, not this class's. What is this
 * class's is the cutoff it hands down, and the only way to get that wrong is to push it the wrong
 * way: a grace ADDED to now would collect keys that have not expired yet, and each one of those is a
 * retry of `POST /orders` turned into a second order.
 */
