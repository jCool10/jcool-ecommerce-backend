...
describe('bucket layout changes', () => {
  ...
  //   wrong key does.
  // Violated at: packages/identity/src/email-bucket.ts:25-26 — `identityKeyFingerprint` HMACs a
  //   fixed sentinel under the key and nothing else, so `BUCKET_COUNT` never reaches the pin and the
  //   fingerprint is byte-identical across a layout change. That leaves only the single-row canary in
  //   apps/checkout-core/src/modules/user/infrastructure/identity-bucket-key.verifier.ts:105-113, and
  //   under a halved layout roughly half of all addresses land in the same bucket either way — so
  //   whether the boot is refused is decided by which user happened to register last.
