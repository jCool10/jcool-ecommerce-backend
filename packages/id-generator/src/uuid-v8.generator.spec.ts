...
// The floor is the last timestamp a node's previous holder may have minted at.
describe('uuid-v8 generator floor', () => {
  const FLOOR_MS = START_MS + 5_000;

  it('mints strictly above the floor while the clock still reads below it', () => {
    const generator = UuidV8Generator.createWithClock({ nodeId: 7, clock: fakeClock().clock, floorMs: FLOOR_MS });

    expect(decode(generator.generate(BUCKET)).tsMs).toBe(FLOOR_MS + 1);
  });

  it('keeps counting the sequence while the clock stays below the floor', () => {
    const fake = fakeClock();
    const generator = UuidV8Generator.createWithClock({ nodeId: 7, clock: fake.clock, floorMs: FLOOR_MS });
  ...
  it('ignores a floor the clock is already past', () => {
    const generator = UuidV8Generator.createWithClock({ nodeId: 7, clock: fakeClock().clock, floorMs: START_MS - 1 });

    expect(decode(generator.generate(BUCKET)).tsMs).toBe(START_MS);
  });

  it('does not report the lift as clock drift', () => {
    const generator = UuidV8Generator.createWithClock({ nodeId: 7, clock: fakeClock().clock, floorMs: FLOOR_MS });
  ...
  it('rejects a floor the timestamp field cannot exceed', () => {
    for (const floorMs of [-1, 1.5, MAX_TIMESTAMP_MS]) {
      expect(() => UuidV8Generator.create({ nodeId: 0, floorMs })).toThrow(RangeError);
    }
  });
});
