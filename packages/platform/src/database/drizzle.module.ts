import { type DynamicModule, Inject, Module, OnApplicationShutdown } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { drizzle } from 'drizzle-orm/node-postgres';
import { ClsService } from 'nestjs-cls';
import { PinoLogger } from 'nestjs-pino';
import { Pool } from 'pg';
import { createDbQueryCounterLogger } from '../observability';
import { toError } from '@jcool/kernel';
import { DRIZZLE, PG_POOL, type DrizzleDBOf, type DrizzleSchema } from './drizzle.tokens';

@Module({})
export class DrizzleModule implements OnApplicationShutdown {
  constructor(@Inject(PG_POOL) private readonly pool: Pool) {}

  static forRoot<S extends DrizzleSchema>(options: { schema: S }): DynamicModule {
    return {
      ...
      providers: [
        ...
        },
        {
          ...
          useFactory: (pool: Pool, cls: ClsService): DrizzleDBOf<S> =>
            drizzle(pool, { schema: options.schema, logger: createDbQueryCounterLogger(cls) }),
        },
      ],
      exports: [DRIZZLE],
    };
