...
describe('ResilienceModule', () => {
  it('resolves the factory against the shipped configuration', async () => {
    const moduleRef = await Test.createTestingModule({
      imports: [
        ConfigModule.forRoot({ load: [resilienceConfig], ignoreEnvFile: true, isGlobal: true }),
    ...
    // provider proves the six keys are spelled the same here and in resilience.config.ts.
